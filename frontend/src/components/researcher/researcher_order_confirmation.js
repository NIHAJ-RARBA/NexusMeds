...
const RESEARCHER_ORDER_CONFIRMATION = ({ isOpen, toggle, price, userId, prescription }) => {
    ...
    const getResearcher = async () => {
        try {
            const response = await fetch(`http://localhost:5000/researcher/get/${userId}`);
            if (!response.ok) {
                throw new Error('Network response was not ok');
            }
            const jsonData = await response.json();
            console.log('inside getResearcher');
...
export default RESEARCHER_ORDER_CONFIRMATION;
